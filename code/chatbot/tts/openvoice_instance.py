...
class OpenVoiceTTS:
    ...
    def speak(self, text_to_speak, CONVERSATION_LOG):
        ...
        if OPENVOICE_MODEL:
            try:
                # Generate speech with more expressive style
                OPENVOICE_MODEL.tts(
                    text=text_to_speak,
                    output_path=OPENVOICE_OUTPUT_AUDIO_PATH,
                    speaker='cheerful',  # Using cheerful style for more expressiveness
                    language='English', # Can be 'en', 'es', 'fr', 'zh', 'jp', 'kr'
                    speed=0.9  # Slightly slower speed for better tonality
                )
